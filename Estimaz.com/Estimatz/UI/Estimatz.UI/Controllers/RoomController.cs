...
using Estimatz.Entities.Player;
...
namespace Estimatz.UI.Controllers
{
    ...
    {
        public RoomController(IHttpContextAccessor httpContextAccessor, IMediator mediatr, IMapper mapper, INotificator notification) : base(mapper, mediatr, httpContextAccessor, notification) { }
        ...
        public async Task<IActionResult> Rooms()
        {
            ...
            var simpleRooms = await _mediatr.Send(new GetSimpleRoomsQuery { UserId = Guid.Parse(userIdString) });
        ...
        public async Task<IActionResult> SaveRoomConfig(RoomConfigModel model) //TODO: Achar uma maneira de proteger esse endpoint
        {
            ...
            var command = _mapper.Map<SaveRoomCommand>(room);
        ...
        public async Task<IActionResult> DeleteRoom(Guid id)
        {
            ...
            var userIdString = _session.GetString("userId");
            await _mediatr.Send(new DeleteRoomCommand { RoomId = id, UserId = Guid.Parse(userIdString) });

            return Json(new { success = _notificator.IsSuccess });
        }
        ...
        public async Task<IActionResult> UpdateStoryStatus(int newStatus, Guid storyId, Guid roomId)
        {
            ...
            await _mediatr.Send(new UpdateStatusStoryCommand { NewStoryStatus = status, RoomId = roomId, StoryId = storyId });
