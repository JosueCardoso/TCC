    ...
    public class UpdateStatusStoryCommandHandler : IRequestHandler<UpdateStatusStoryCommand>
    {
        ...
        public async Task Handle(UpdateStatusStoryCommand request, CancellationToken cancellationToken)
        {
            ...
            if(room is not null)
            {
                ...
                if (indexArray != -1)
                {
                    var newRoomStatus = Entity.RoomStatus.NotStarted;
                    bool updateRoomStatus = false;

                    if(request.NewStoryStatus == StoryStatus.InProgress && room.Status == Entity.RoomStatus.NotStarted)
                    {
                        newRoomStatus = Entity.RoomStatus.Unfinished;
                        updateRoomStatus = true;
                    }                    
                        
                    if(request.NewStoryStatus == StoryStatus.Finished && room.Status == Entity.RoomStatus.Unfinished && CanFinishRoom(room, request.StoryId))
                    {
                        newRoomStatus = Entity.RoomStatus.Finished;
                        updateRoomStatus = true;
                    }
        ...
        private bool CanFinishRoom(Entity.Room room, Guid storyId)
        {
