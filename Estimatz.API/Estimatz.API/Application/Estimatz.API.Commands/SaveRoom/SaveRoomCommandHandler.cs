    ...
    public class SaveRoomCommandHandler : IRequestHandler<SaveRoomCommand, Guid>
    {
        ...
        public async Task<Guid> Handle(SaveRoomCommand request, CancellationToken cancellationToken)
        {
            ...
            if (response?.StatusCode == System.Net.HttpStatusCode.Created)
            {
                ...
                if (room.RoomConfig.VotingType == VotingType.FreeVoting) //Caso a votação seja livre (sem tasks ou story) deve ser criado ao menos uma por baixo dos panos para que seja possível armazenar as informações das votações
                    CreateStory(room.Id);
